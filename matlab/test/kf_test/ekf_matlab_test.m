...
ekf = extendedKalmanFilter(@model.stateTransFun, @model.msrFun, x0_hat);
ekf.MeasurementNoise = R_hat;
ekf.ProcessNoise = Q;
ekf.StateCovariance = P0;
% ekf.StateTransitionJacobianFcn = @model.stateTransFunJacob;
% ekf.MeasurementJacobianFcn = @model.msrFunJacob;
...
for k=1:n_steps
    ...
    e(:,k) = y_data(:,k) - model.msrFun(ekf.State); % ukf.State is x[k|k-1] at this point
...
for i=1:n_dim
    ...
    if (i==1), title('EKF matlab estimation results', 'interpreter','latex', 'fontsize',17); end
    hold off;
end
