...
for i=1:3
    subplot(5,1,i);
    hold on;
    plot([Time(1) Time(end)],[0 0], 'LineStyle','--', 'Color',goal_color{i} ,'LineWidth',2);
    plot(Time,eQg_data(i,:), 'LineStyle','-', 'Color',goal_est_color{i}, 'LineWidth',linewidth);
    plot(Time,eQ_data(i,:), 'LineStyle','-.', 'Color',pos_color{i}, 'LineWidth',linewidth);
...
%     legend_labels = {};
%     for i=1:D
%         plot([Time(1) Time(end)],[Qg(i) Qg(i)], 'LineStyle','--', 'Color',goal_color{i} ,'LineWidth',2, 'Parent',est_fig_ax{1});
%         plot(Time,Qg_data(i,:), 'LineStyle','-', 'Color',goal_est_color{i}, 'LineWidth',linewidth, 'Parent',est_fig_ax{1});
%         plot(Time,Q_data(i,:), 'LineStyle','-.', 'Color',pos_color{i}, 'LineWidth',linewidth, 'Parent',est_fig_ax{1});
%         legend_labels = [legend_labels, ['$\mathbf{Q}_{g,' axis_name{i} '}$'], ['$\hat{\mathbf{Q}}_{g,' axis_name{i} '}$'], ['$\mathbf{Q}_{' axis_name{i} '}$']];
%         if (plot_1sigma)
%             plot(Time,Qg_data(i,:)+Sigma_theta_data(i,:),'c-.', 'LineWidth',linewidth, 'Parent',est_fig_ax{1});
%         	plot(Time,Qg_data(i,:)-Sigma_theta_data(i,:),'c-.', 'LineWidth',linewidth, 'Parent',est_fig_ax{1});
%             legend_labels = [legend_labels, ['$\pm1\sigma$']];
%         end
...
% plot([t_yg t_yg], y_lim, 'LineStyle','--', 'Color',[0.85 0.33 0.1], 'LineWidth',1.5, 'Parent',ax);%, 'HandleVisibility','off');
% plot([t_y t_y], y_lim, 'LineStyle','--', 'Color',[0 0 0], 'LineWidth',1.5, 'Parent',ax);%, 'HandleVisibility','off');
% legend_labels = {['$||\hat{\mathbf{Q}}_{g}-\mathbf{Q}_{g}||$'], ['$||\mathbf{Q}-\mathbf{Q}_{g}||$'], ...
%     'estimate $t_{s}$', 'pos $t_s$'};
% legend(ax, legend_labels,'interpreter','latex','fontsize',fontsize, 'Orientation','horizontal');
