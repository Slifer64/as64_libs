...
UrCommunication::UrCommunication(ur_::Semaphore &msg_sem, std::string host)
{
  this->msg_sem_ptr = &msg_sem;
	...
	if (pri_sockfd_ < 0) print_fatal("ERROR opening socket pri_sockfd");
	sec_sockfd_ = socket(AF_INET, SOCK_STREAM, 0);
	if (sec_sockfd_ < 0) print_fatal("ERROR opening socket sec_sockfd");
	server_ = gethostbyname(host.c_str());
	if (server_ == NULL) print_fatal("ERROR, unknown host");
	...
	while (keepalive_)
	{
			...
			else
      {
		...
		if (keepalive_)
		{
			...
			sec_sockfd_ = socket(AF_INET, SOCK_STREAM, 0);
			if (sec_sockfd_ < 0) print_fatal("ERROR opening secondary socket");
			flag_ = 1;
			setsockopt(sec_sockfd_, IPPROTO_TCP, TCP_NODELAY, (char *) &flag_, sizeof(int));
			setsockopt(sec_sockfd_, IPPROTO_TCP, TCP_QUICKACK, (char *) &flag_, sizeof(int));
			setsockopt(sec_sockfd_, SOL_SOCKET, SO_REUSEADDR, (char *) &flag_, sizeof(int));
